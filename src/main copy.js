...
camera.position.set(-4,12.5,20)
camera.rotation.set(-0.35885993397896615,-0.20114612999000137,-0.07480269320403296)
...
loader.load(
  ...
  function (gltf) {
    ...
    ironman.traverse(child=>{
      ...
      if(child.name =='helmet_front'){
        helmet = child
      }
    })

    const helmet_contoler = sheet.object("helmet",{
      position:{
        x:types.number(0,{range:[0,50],nudgeMultiplier:0.01}),
        y:types.number(0,{range:[0,50],nudgeMultiplier:0.01}),
        z:types.number(0,{range:[0,50],nudgeMultiplier:0.01}),
      }
    })
    helmet_contoler.onValuesChange(value=>{
      helmet.position.set(value.position.x,value.position.y,value.position.z)
    })
...
//keylight
const keyLight = new THREE.RectAreaLight(0xffffff,1.5,20,20)
...
//keylight2
const keyLight2 = new THREE.RectAreaLight(0xffffff,2,20,20)
keyLight2.position.set(0,5,14)
...
//oribit controls
const controls = new OrbitControls(scene_camera,renderer.domElement)
